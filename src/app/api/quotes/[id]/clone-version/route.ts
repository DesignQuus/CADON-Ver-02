...
export async function POST(
  ...
  const oldQuote = db.prepare('SELECT * FROM quotes WHERE id = ?').get(id) as any;
  if (!oldQuote) {
    return NextResponse.json({ error: '견적서를 찾을 수 없습니다.' }, { status: 404 });
  }

  try {
    const oldItems = db.prepare('SELECT * FROM quote_items WHERE quote_id = ? ORDER BY item_no ASC').all(id) as any[];
