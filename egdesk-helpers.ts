 ...
 * internal knowledge / business identity / company research (MCP), Korean law (MCP), Bizinfo grants (MCP), NPS workplace headcount (MCP), and browser recorder replay / live browse.
 ...
 * - updateRows('table', { col: 'val' }, { filters: { id: '1' } })
 * - updateRows('table', { updates: { col: 'val' }, filters: { id: '1' } })
 */
export async function updateRows(
  tableName: string,
  updatesOrPayload: Record<string, any>,
  options?: {
    ids?: number[];
    filters?: Record<string, string>;
  }
  ...
  let rowOptions: typeof options;

  if (
    options === undefined &&
    updatesOrPayload !== null &&
    typeof updatesOrPayload === 'object' &&
    'updates' in updatesOrPayload &&
    updatesOrPayload.updates !== null &&
    typeof updatesOrPayload.updates === 'object' &&
    !Array.isArray(updatesOrPayload.updates)
  ) {
    const payload = updatesOrPayload as {
      updates: Record<string, any>;
      ids?: number[];
      filters?: Record<string, string>;
    };
    updates = payload.updates;
    rowOptions = { ids: payload.ids, filters: payload.filters };
  } else {
    updates = updatesOrPayload;
    rowOptions = options;
  }
...
export async function searchBizinfo(
  options: {
    category?: 'finance' | 'tech' | 'talent' | 'export' | 'domestic' | 'startup' | 'management' | 'other';
    region?: string;
    hashtags?: string;
    query?: string;
    display?: number;
  ...
  return callBizinfoTool('bizinfo_search', options);
}

/** Currently open 지원사업, sorted by deadline */
export async function listOpenBizinfo(
  options: {
    category?: 'finance' | 'tech' | 'talent' | 'export' | 'domestic' | 'startup' | 'management' | 'other';
    region?: string;
    hashtags?: string;
  ...
  return callBizinfoTool('bizinfo_open', options);
}

/** Get one announcement by pblancId */
export async function getBizinfoAnnouncement(id: string) {
  return callBizinfoTool('bizinfo_get', { id });
}
...
export async function searchNpsWorkplaces(
  options: {
  ...
  return callNpsTool('nps_search', options);
}

/** NPS workplace detail (가입자수, 당월고지금액) by seq */
export async function getNpsWorkplaceDetail(seq: string) {
  return callNpsTool('nps_detail', { seq });
}

/** Monthly subscriber trend by seq */
export async function getNpsWorkplaceTrend(seq: string) {
  return callNpsTool('nps_trend', { seq });
}

/** Name (+ optional 사업자번호) → latest month + detail + 가입자 추이 */
export async function lookupNpsWorkplace(options: {
  workplaceName: string;
  businessNumber?: string;
  sidoCode?: string;
  display?: number;
}) {
  return callNpsTool('nps_lookup', options);
}

// ==========================================
// KONEPS / 나라장터 계약정보 (MCP)
// ==========================================

export async function callKonepsTool(
  toolName: string,
  args: Record<string, any> = {}
): Promise<any> {
  const body = JSON.stringify({ tool: toolName, arguments: args });
...
export async function searchKonepsContracts(options: Record<string, any> = {}) {
  return callKonepsTool('koneps_search', options);
}

export async function getKonepsContract(untyCntrctNo: string, category?: string) {
  return callKonepsTool('koneps_get', { untyCntrctNo, category });
}

export async function lookupKonepsVendor(options: {
  companyName?: string;
  businessNumber?: string;
  startDate?: string;
  endDate?: string;
  ...
  return callKonepsTool('koneps_lookup', options);
}

// ==========================================
// BidNotice / 나라장터 입찰공고 (MCP)
// ==========================================

export async function callBidnoticeTool(
  toolName: string,
  args: Record<string, any> = {}
): Promise<any> {
  const body = JSON.stringify({ tool: toolName, arguments: args });
...
export async function searchBidNotices(options: Record<string, any> = {}) {
  return callBidnoticeTool('bidnotice_search', options);
}

export async function getBidNotice(noticeNo: string, category?: string) {
  return callBidnoticeTool('bidnotice_get', { noticeNo, category });
}

export async function lookupBidNotices(options: {
  title?: string;
  institutionName?: string;
  institutionDiv?: string;
  startDate?: string;
  endDate?: string;
  category?: string;
  openOnly?: boolean;
  ...
  return callBidnoticeTool('bidnotice_lookup', options);
}
...
export const AI_KEY_NAMES = ['egdesk-gemini'] as const;
